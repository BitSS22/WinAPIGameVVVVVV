...
void UEngineFile::Write(const void* _Ptr, size_t _Size)
{
    if (_Size == 0)
    {
        assert(nullptr, "크기가 0인 데이터를 쓸 수 없습니다.");
        return;
    }
    else if (_Ptr == nullptr)
    {
        assert(nullptr, "존재하지 않는 메모리를 사용할 수 없습니다.");
        return;
    }
    else if (File == nullptr)
    {
        assert(nullptr, "열지 않은 파일에 내용을 쓸 수 없습니다");
        return;
    }
...
void UEngineFile::Read(void* _Ptr, size_t _Size)
{
    if (_Size == 0)
    {
        assert(nullptr, "크기가 0인 데이터를 읽을 수 없습니다.");
        return;
    }
    else if (_Ptr == nullptr)
    {
    ...
    {
        assert(nullptr, "열지 않은 파일의 내용을 읽을 수 없습니다.");
        return;
    }
...
void UEngineFile::FileOpen(const char* _Mode)
{
    ...
    if (File == nullptr)
        assert(nullptr, "유효하지 않은 Path입니다.");
}
